...
if( file_exists('custom/modules/Flats_items/metadata/listviewdefs.php') )
{
	require_once('custom/modules/Flats_items/metadata/listviewdefs.php');
}
else
{
	require_once('modules/Flats_items/metadata/listviewdefs.php');
}
...
$current_module_strings = return_module_language($current_language, 'Flats_items');
...
$seedFlats_item = new Flats_item();

$searchForm = new SearchForm('Flats_items', $seedFlats_item);
...
if(!empty($_REQUEST['displayColumns']))
{
	...
	{
        	if(!empty($listViewDefs['Flats_items'][$col]))
		{
            		$displayColumns[$col] = $listViewDefs['Flats_items'][$col];
		}
	}
}
...
{
	foreach( $listViewDefs['Flats_items'] as $col => $params )
	{
...
if( isset($_REQUEST['query']) )
{
    	...
    	else
	{
    	...
    	$where_clauses = $searchForm->generateSearchWhere(true, "Flats_items");
...
$lv->setup($seedFlats_item, 'include/ListView/ListViewGeneric.tpl', $where, $params);
